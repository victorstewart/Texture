...
- (void)didLoad {
  ...
  void (^configureTextView)(UITextView *) = ^(UITextView *textView) {
    if (!_displayingPlaceholder || textView != _textKitComponents.textView) {
      // If showing the placeholder, don't propagate backgroundColor/opaque to the editable textView.  It is positioned
      // over the placeholder to accept taps to begin editing, and if it's opaque/colored then it'll obscure the
      // placeholder.
      textView.backgroundColor = self.backgroundColor;
      textView.opaque = self.opaque;
    } else if (_displayingPlaceholder && textView == _textKitComponents.textView) {
      // The default backgroundColor for a textView is white.  Due to the reason described above, make sure the editable
      // textView starts out transparent.
  ...
  _placeholderTextKitComponents.textView =
      [[ASTextKitComponentsTextView alloc] initWithFrame:CGRectZero
                                           textContainer:_placeholderTextKitComponents.textContainer];
  ...
  _textKitComponents.textView = [[ASPanningOverriddenUITextView alloc] initWithFrame:CGRectZero
                                                                       textContainer:_textKitComponents.textContainer];
...
- (void)setBackgroundColor:(UIColor *)backgroundColor {
  ...
  // If showing the placeholder, don't propagate backgroundColor/opaque to the editable textView.  It is positioned over
  // the placeholder to accept taps to begin editing, and if it's opaque/colored then it'll obscure the placeholder. The
  // backgroundColor/opaque will be propagated to the editable textView when editing begins.
...
- (UITextView *)textView {
  ...
  ASDisplayNodeAssert(_textKitComponents.textView != nil,
                      @"UITextView must be created in -[ASEditableTextNode didLoad]");
...
- (void)setAttributedText:(NSAttributedString *)attributedText {
  ...
  // storage and expect perfect propagation to the text view. Thus, we always update the textview directly if it's been
  // created already.
  ...
  if (preserveSelectedRange) {
    _isPreservingSelection =
        YES;  // Used in -textViewDidChangeSelection: to avoid informing our delegate about our preservation.
...
- (void)_updateDisplayingPlaceholder {
  ...
  // If hiding the placeholder, propagate backgroundColor/opaque to the editable textView.  It is positioned over the
  // placeholder to accept taps to begin editing, and was kept transparent so it doesn't obscure the placeholder text.
  // Now that we're editing it and the placeholder is hidden, we can make it opaque to avoid unnecessary blending.
  if (!_displayingPlaceholder) {
...
- (void)_layoutTextView {
  ...
  // When we resize to fit (above) the prior layout becomes invalid. For whatever reason, UITextView doesn't invalidate
  // its layout when its frame changes on its own, so we have to do so ourselves.
  [_textKitComponents.layoutManager
      invalidateLayoutForCharacterRange:NSMakeRange(0, [_textKitComponents.textStorage length])
...
- (void)textViewDidChange:(UITextView *)textView {
  ...
  // changes that happen because of editing or pure selection changes.
...
- (CGRect)frameForTextRange:(NSRange)textRange {
  ...
  NSRange glyphRange = [_textKitComponents.layoutManager glyphRangeForCharacterRange:textRange
                                                                actualCharacterRange:NULL];
  CGRect textRect = [_textKitComponents.layoutManager boundingRectForGlyphRange:glyphRange
                                                                inTextContainer:_textKitComponents.textContainer];
...
- (void)_delegateDidChangeSelectionFromSelectedRange:(NSRange)fromSelectedRange
  ...
  // hierarchy.
  // 2. This delegate method (-textViewDidChangeSelection:) is called both before -textViewDidChange: and before the
  // layout manager/etc. has necessarily generated+laid out its glyphs. Because of the former, we need to wait until
  // -textViewDidChange: has had an opportunity to be called so can accurately determine whether this selection change
  // is due to editing (_selectionChangedForEditedText). Thus, to avoid calling out to client code in the middle of
  // UITextView's processing, we call the delegate on the next run of the runloop, when all such internal processing is
  // surely done.
  dispatch_async(dispatch_get_main_queue(), ^{
    if ([_delegate respondsToSelector:@selector(editableTextNodeDidChangeSelection:
                                                                 fromSelectedRange:toSelectedRange:dueToEditing:)])
      [_delegate editableTextNodeDidChangeSelection:self
                                  fromSelectedRange:fromSelectedRange
                                    toSelectedRange:toSelectedRange
                                       dueToEditing:_selectionChangedForEditedText];
  });
}
...
- (void)_delegateDidUpdateText {
  ...
  // UITextView invokes its delegate methods when it's in the middle of text-processing. For example,
  // -textViewDidChange: is invoked before you can truly rely on the changes being propagated throughout the Text Kit
  // hierarchy. Thus, to avoid calling out to client code in the middle of UITextView's processing, we call the delegate
  // on the next run of the runloop, when all such internal processing is surely done.
  dispatch_async(dispatch_get_main_queue(), ^{
