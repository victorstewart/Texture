...
#ifdef __GNUC__
#define ASDISPLAYNODE_GNUC(major, minor) (__GNUC__ > (major) || (__GNUC__ == (major) && __GNUC_MINOR__ >= (minor)))
#else
...
#define ASCompareAssignObjects(lvalue, newValue) ASCompareAssignCustom(lvalue, newValue, ASObjectIsEqual)
...
#define ASCompareAssignCopy(lvalue, newValue)         \
  ({                                                  \
    ...
    if (result) {                                     \
      lvalue = [newValue copyWithZone:NULL];          \
    }                                                 \
    result;                                           \
  })
