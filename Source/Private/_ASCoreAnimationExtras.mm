...
void ASDisplayNodeSetResizableContents(id<ASResizableContents> obj, UIImage *image) {
  ...
  // use CALayer directly to perform such tiling on the GPU, though the stretch is handled by the GPU, and CALayer.h
  // documents the fact that contentsCenter is used to stretch the pixels.

  if (image) {
    ASDisplayNodeCAssert(image.resizingMode == UIImageResizingModeStretch ||
                             UIEdgeInsetsEqualToEdgeInsets(image.capInsets, UIEdgeInsetsZero),
                         @"Image insets must be all-zero or resizingMode has to be UIImageResizingModeStretch. XCode "
                         @"assets default value is UIImageResizingModeTile which is not supported by Texture because "
                         @"of GPU-accelerated CALayer features.");
    ...
    // These are lifted from what UIImageView does by experimentation. Without these exact values, the stretching is
    // slightly off.
    ...
    if (insets.left > 0 || insets.right > 0) {
      ...
      contentsCenter.size.width =
          (imageSize.width - (insets.left + insets.right + 1.f) + otherPixelFudge) / imageSize.width;
    }
    if (insets.top > 0 || insets.bottom > 0) {
      contentsCenter.origin.y = ((insets.top + halfPixelFudge) / imageSize.height);
      contentsCenter.size.height =
          (imageSize.height - (insets.top + insets.bottom + 1.f) + otherPixelFudge) / imageSize.height;
    }
