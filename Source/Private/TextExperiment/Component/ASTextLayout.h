...
/// YES:(PathFillEvenOdd) Text is filled in the area that would be painted if the path were given to
/// CGContextEOFillPath. NO: (PathFillWindingNumber) Text is fill in the area that would be painted if the path were
/// given to CGContextFillPath. Default is YES;
 ...
 */
- (void)modifyLines:(NSArray<ASTextLine *> *)lines
           fromText:(NSAttributedString *)text
        inContainer:(ASTextContainer *)container;
@end
...
/**
 ...
 @return A new layout, or nil when an error occurs.
 */
+ (nullable ASTextLayout *)layoutWithContainer:(ASTextContainer *)container
...
/**
 ...
 @param debug   The debug option. Pass nil to avoid debug drawing.
 */
- (void)drawInContext:(nullable CGContextRef)context size:(CGSize)size debug:(nullable ASTextDebugOption *)debug;
