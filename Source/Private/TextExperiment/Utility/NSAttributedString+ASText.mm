...
+ (NSMutableAttributedString *)as_attachmentStringWithEmojiImage:(UIImage *)image fontSize:(CGFloat)fontSize
{
  ...
  attachment.contentInsets = UIEdgeInsetsMake(ascent - (bounding.size.height + bounding.origin.y),
                                              bounding.origin.x,
                                              descent + bounding.origin.y,
                                              bounding.origin.x);
  if (hasAnim) {
                 ...
                 longPressAction:(ASTextAction)longPressAction
{
  ...
  if (color)
    [self as_setColor:color range:range];
...
+ (NSArray *)as_allDiscontinuousAttributeKeys
{
  ...
  dispatch_once(&onceToken, ^{
    keys = @[
      (id)kCTSuperscriptAttributeName,
