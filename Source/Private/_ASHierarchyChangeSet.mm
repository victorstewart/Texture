...
#define ASFailUpdateValidation(...)                                                                                   \
  _Pragma("clang diagnostic push") _Pragma(                                                                           \
      "clang diagnostic ignored \"-Wdeprecated-declarations\"") if ([ASDisplayNode                                    \
...
- (void)_sortAndCoalesceChangeArrays
{
  ...
  @autoreleasepool {
    ...
    for (_ASHierarchyItemChange *change in _reloadItemChanges) {
      ...
      const auto newIndexPaths = ASArrayByFlatMapping(
          change.indexPaths, NSIndexPath * indexPath, [self newIndexPathForOldIndexPath:indexPath]);
...
- (void)_validateUpdate
{
  ...
  if (newSectionCount != oldSectionCount + insertedSectionCount - deletedSectionCount) {
    ASFailUpdateValidation(@"Invalid number of sections. The number of sections after the update (%ld) must be equal "
                           ...
                           (long)newSectionCount,
                           (long)oldSectionCount,
                           (long)insertedSectionCount,
  ...
  for (_ASHierarchyItemChange *change in _insertItemChanges) {
    for (NSIndexPath *indexPath in change.indexPaths) {
      ...
      if (item >= newItemCount) {
        ASFailUpdateValidation(
            @"Attempt to insert item %ld into section %ld, which only contains %ld items after the update.",
            (long)item,
            (long)section,
            (long)newItemCount);
        return;
...
+ (NSString *)smallDescriptionForItemChanges:(NSArray<_ASHierarchyItemChange *> *)changes
{
  ...
  [map enumerateKeysAndObjectsUsingBlock:^(
           NSNumber *_Nonnull section, NSIndexSet *_Nonnull indexSet, BOOL *_Nonnull stop) {
