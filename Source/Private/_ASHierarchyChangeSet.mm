...
#define ASFailUpdateValidation(...)                                                                       \
  _Pragma("clang diagnostic push") _Pragma("clang diagnostic ignored \"-Wdeprecated-declarations\"") if ( \
...
- (instancetype)initWithChangeType:(_ASHierarchyChangeType)changeType
                          indexSet:(NSIndexSet *)indexSet
                  animationOptions:(ASDataControllerAnimationOptions)animationOptions;
...
/**
 ...
 Assumes: `changes` all have the same changeType
 */
+ (void)sortAndCoalesceItemChanges:(NSMutableArray<_ASHierarchyItemChange *> *)changes
         ignoringChangesInSections:(NSIndexSet *)sections;
...
- (ASIntegerMap *)sectionMapping {
  ...
  if (_sectionMapping == nil) {
    _sectionMapping = [ASIntegerMap mapForUpdateWithOldCount:_oldItemCounts.size()
                                                     deleted:_deletedSections
                                                    inserted:_insertedSections];
  }
  ...
  if (_itemMappings == nil) {
    ...
    for (NSInteger oldCount : _oldItemCounts) {
      ...
      if (newSection == NSNotFound) {
        ...
        table = [ASIntegerMap mapForUpdateWithOldCount:oldCount
                                               deleted:deleteMap[@(oldSection)]
                                              inserted:insertMap[@(newSection)]];
      }
...
- (void)deleteSections:(NSIndexSet *)sections animationOptions:(ASDataControllerAnimationOptions)options {
  [self _ensureNotCompleted];
  _ASHierarchySectionChange *change =
      [[_ASHierarchySectionChange alloc] initWithChangeType:_ASHierarchyChangeTypeOriginalDelete
                                                   indexSet:sections
                                           animationOptions:options];
...
- (void)insertItems:(NSArray *)indexPaths animationOptions:(ASDataControllerAnimationOptions)options {
  [self _ensureNotCompleted];
  _ASHierarchyItemChange *change =
      [[_ASHierarchyItemChange alloc] initWithChangeType:_ASHierarchyChangeTypeOriginalInsert
                                              indexPaths:indexPaths
                                        animationOptions:options
                                               presorted:NO];
...
- (void)insertSections:(NSIndexSet *)sections animationOptions:(ASDataControllerAnimationOptions)options {
  [self _ensureNotCompleted];
  _ASHierarchySectionChange *change =
      [[_ASHierarchySectionChange alloc] initWithChangeType:_ASHierarchyChangeTypeOriginalInsert
                                                   indexSet:sections
                                           animationOptions:options];
...
- (void)reloadItems:(NSArray *)indexPaths animationOptions:(ASDataControllerAnimationOptions)options {
  [self _ensureNotCompleted];
  _ASHierarchyItemChange *change = [[_ASHierarchyItemChange alloc] initWithChangeType:_ASHierarchyChangeTypeReload
                                                                           indexPaths:indexPaths
                                                                     animationOptions:options
                                                                            presorted:NO];
...
- (void)reloadSections:(NSIndexSet *)sections animationOptions:(ASDataControllerAnimationOptions)options {
  [self _ensureNotCompleted];
  _ASHierarchySectionChange *change = [[_ASHierarchySectionChange alloc] initWithChangeType:_ASHierarchyChangeTypeReload
                                                                                   indexSet:sections
                                                                           animationOptions:options];
...
- (void)_sortAndCoalesceChangeArrays {
  ...
  @autoreleasepool {
    ...
    for (_ASHierarchySectionChange *change in _reloadSectionChanges) {
      ...
      _ASHierarchySectionChange *deleteChange =
          [[_ASHierarchySectionChange alloc] initWithChangeType:_ASHierarchyChangeTypeDelete
                                                       indexSet:change.indexSet
                                               animationOptions:change.animationOptions];
      [_deleteSectionChanges addObject:deleteChange];

      _ASHierarchySectionChange *insertChange =
          [[_ASHierarchySectionChange alloc] initWithChangeType:_ASHierarchyChangeTypeInsert
                                                       indexSet:newSections
                                               animationOptions:change.animationOptions];
    ...
    for (_ASHierarchyItemChange *change in _reloadItemChanges) {
      ...
      const auto newIndexPaths = ASArrayByFlatMapping(change.indexPaths, NSIndexPath * indexPath,
                                                      [self newIndexPathForOldIndexPath:indexPath]);
      ...
      _ASHierarchyItemChange *deleteItemChangeFromReloadChange =
          [[_ASHierarchyItemChange alloc] initWithChangeType:_ASHierarchyChangeTypeDelete
                                                  indexPaths:change.indexPaths
                                            animationOptions:change.animationOptions
                                                   presorted:NO];
      ...
      _ASHierarchyItemChange *insertItemChangeFromReloadChange =
          [[_ASHierarchyItemChange alloc] initWithChangeType:_ASHierarchyChangeTypeInsert
...
- (void)_validateUpdate {
  ...
  if (newSectionCount != oldSectionCount + insertedSectionCount - deletedSectionCount) {
    ASFailUpdateValidation(@"Invalid number of sections. The number of sections after the update (%ld) must be equal "
                           @"to the number of sections before the update (%ld) plus or minus the number of sections "
                           @"inserted or deleted (%ld inserted, %ld deleted)",
                           (long)newSectionCount, (long)oldSectionCount, (long)insertedSectionCount,
                           (long)deletedSectionCount);
    return;
  }
  ...
  if (invalidSectionDelete != NSNotFound) {
    ASFailUpdateValidation(@"Attempt to delete section %ld but there are only %ld sections before the update.",
                           (long)invalidSectionDelete, (long)oldSectionCount);
    return;
  }
    ...
    for (NSIndexPath *indexPath in change.indexPaths) {
      ...
      if (item >= oldItemCount) {
        ASFailUpdateValidation(
            ...
            (long)item, (long)section, (long)oldItemCount);
        return;
      }
    }
  }
    ...
    for (NSIndexPath *indexPath in change.indexPaths) {
      ...
      if (section >= newSectionCount) {
        ASFailUpdateValidation(
            ...
            (long)item, (long)section, (long)newSectionCount);
        return;
      }
      ...
      if (item >= newItemCount) {
        ASFailUpdateValidation(
            @"Attempt to insert item %ld into section %ld, which only contains %ld items after the update.", (long)item,
            (long)section, (long)newItemCount);
        return;
      }
    }
  }
  ...
  if (invalidSectionInsert != NSNotFound) {
    ASFailUpdateValidation(@"Attempt to insert section %ld but there are only %ld sections after the update.",
                           (long)invalidSectionInsert, (long)newSectionCount);
    return;
  }
  ...
  for (NSUInteger oldSection = 0; oldSection < oldSectionCount; oldSection++) {
    ...
    NSIndexSet *originalInsertedItems = [self indexesForItemChangesOfType:_ASHierarchyChangeTypeOriginalInsert
                                                                inSection:newSection];
    NSIndexSet *originalDeletedItems = [self indexesForItemChangesOfType:_ASHierarchyChangeTypeOriginalDelete
                                                               inSection:oldSection];
    ...
    if (deletedReloadedItem != NSNotFound) {
      ASFailUpdateValidation(@"Attempt to delete and reload the same item at index path %@",
                             [NSIndexPath indexPathForItem:deletedReloadedItem inSection:oldSection]);
      return;
    }
    ...
    if (newItemCount != oldItemCount + insertedItemCount - deletedItemCount) {
      ASFailUpdateValidation(@"Invalid number of items in section %ld. The number of items after the update (%ld) must "
                             @"be equal to the number of items before the update (%ld) plus or minus the number of "
                             @"items inserted or deleted (%ld inserted, %ld deleted).",
                             (long)oldSection, (long)newItemCount, (long)oldItemCount, (long)insertedItemCount,
...
- (BOOL)_includesPerItemOrSectionChanges {
  return 0 <
         (_originalDeleteSectionChanges.count + _originalDeleteItemChanges.count + _originalInsertSectionChanges.count +
          _originalInsertItemChanges.count + _reloadSectionChanges.count + _reloadItemChanges.count);
}
...
- (NSMutableArray<NSDictionary *> *)propertiesForDescription {
  ...
  if (_reloadSectionChanges.count > 0) {
    [result addObject:@{
      @"reloadSections" : [_ASHierarchySectionChange smallDescriptionForSectionChanges:_reloadSectionChanges]
    }];
  }
  ...
  if (_originalDeleteSectionChanges.count > 0) {
    [result addObject:@{
      @"deleteSections" : [_ASHierarchySectionChange smallDescriptionForSectionChanges:_originalDeleteSectionChanges]
    }];
  }
  if (_originalDeleteItemChanges.count > 0) {
    [result addObject:@{
      @"deleteItems" : [_ASHierarchyItemChange smallDescriptionForItemChanges:_originalDeleteItemChanges]
    }];
  }
  if (_originalInsertSectionChanges.count > 0) {
    [result addObject:@{
      @"insertSections" : [_ASHierarchySectionChange smallDescriptionForSectionChanges:_originalInsertSectionChanges]
    }];
  }
  if (_originalInsertItemChanges.count > 0) {
    [result addObject:@{
      @"insertItems" : [_ASHierarchyItemChange smallDescriptionForItemChanges:_originalInsertItemChanges]
    }];
  }
  return result;
}
...
- (_ASHierarchySectionChange *)changeByFinalizingType {
  ...
  switch (_changeType) {
    ...
    case _ASHierarchyChangeTypeOriginalDelete:
      ...
      break;
    default:
      ASFailUpdateValidation(@"Attempt to finalize section change of invalid type %@.",
                             NSStringFromASHierarchyChangeType(_changeType));
      return self;
  }
  return [[_ASHierarchySectionChange alloc] initWithChangeType:newType
                                                      indexSet:_indexSet
...
+ (void)sortAndCoalesceSectionChanges:(NSMutableArray<_ASHierarchySectionChange *> *)changes {
  ...
  ASDisplayNodeAssert(ASHierarchyChangeTypeIsFinal(type),
                      @"Attempt to sort and coalesce section changes of intermediary type %@. Why?",
                      NSStringFromASHierarchyChangeType(type));
  ...
  [allIndexes enumerateRangesWithOptions:options
                              usingBlock:^(NSRange range, BOOL *_Nonnull stop) {
                                ...
                                for (NSInteger i = start; i != limit; i += increment) {
                                  ...
                                  if (options != currentOptions && currentIndexes.count > 0) {
                                    _ASHierarchySectionChange *change =
                                        [[_ASHierarchySectionChange alloc] initWithChangeType:type
                                                                                     indexSet:[currentIndexes copy]
                                                                             animationOptions:currentOptions];
  ...
  if (currentIndexes.count > 0) {
    _ASHierarchySectionChange *change = [[_ASHierarchySectionChange alloc] initWithChangeType:type
                                                                                     indexSet:[currentIndexes copy]
                                                                             animationOptions:currentOptions];
    [result addObject:change];
  }
...
- (_ASHierarchyItemChange *)changeByFinalizingType {
  ...
  switch (_changeType) {
    ...
    case _ASHierarchyChangeTypeOriginalDelete:
      ...
      break;
    default:
      ASFailUpdateValidation(@"Attempt to finalize item change of invalid type %@.",
                             NSStringFromASHierarchyChangeType(_changeType));
      return self;
  }
  return [[_ASHierarchyItemChange alloc] initWithChangeType:newType
...
+ (void)sortAndCoalesceItemChanges:(NSMutableArray<_ASHierarchyItemChange *> *)changes
         ignoringChangesInSections:(NSIndexSet *)ignoredSections {
  if (changes.count < 1) {
    return;
  }
  ...
  ASDisplayNodeAssert(ASHierarchyChangeTypeIsFinal(type),
                      @"Attempt to sort and coalesce item changes of intermediary type %@. Why?",
                      NSStringFromASHierarchyChangeType(type));
  ...
  for (NSIndexPath *indexPath in allIndexPaths) {
    ...
    if (options != currentOptions && currentIndexPaths.count > 0) {
      _ASHierarchyItemChange *change = [[_ASHierarchyItemChange alloc] initWithChangeType:type
                                                                               indexPaths:[currentIndexPaths copy]
                                                                         animationOptions:currentOptions
                                                                                presorted:YES];
      [result addObject:change];
...
+ (NSString *)smallDescriptionForItemChanges:(NSArray<_ASHierarchyItemChange *> *)changes {
  ...
  [map enumerateKeysAndObjectsUsingBlock:^(NSNumber *_Nonnull section, NSIndexSet *_Nonnull indexSet,
                                           BOOL *_Nonnull stop) {
