...
/**
 * The following macros are conveniences to help in the common tasks related to the bridging that ASDisplayNode does to
 * UIView and CALayer. In general, a property can either be:
 ...
 *       manually implement
 *
 *  _bridge_prologue_write is defined to take the node's property lock. Add it at the beginning of any bridged property
 * setters. _bridge_prologue_read is defined to take the node's property lock and enforce thread affinity. Add it at the
 * beginning of any bridged property getters.
 */
...
#define _setToViewOrLayer(layerProperty, layerValueExpr, viewAndPendingViewStateProperty, viewAndPendingViewStateExpr) \
  BOOL shouldApply = ASDisplayNodeShouldApplyBridgedWriteToView(self);                                                 \
  if (shouldApply) {                                                                                                   \
    (_view ? _view.viewAndPendingViewStateProperty = (viewAndPendingViewStateExpr)                                     \
           : _layer.layerProperty = (layerValueExpr));                                                                 \
  } else {                                                                                                             \
    ASDisplayNodeGetPendingState(self).viewAndPendingViewStateProperty = (viewAndPendingViewStateExpr);                \
  }
...
/**
 * This category implements certain frequently-used properties and methods of UIView and CALayer so that ASDisplayNode
 * clients can just call the view/layer methods on the node, with minimal loss in performance.  Unlike UIView and
 * CALayer methods, these can be called from a non-main thread until the view or layer is created. This allows text
 * sizing in -calculateSizeThatFits: (essentially a simplified layout) to happen off the main thread without any CALayer
 * or UIView actually existing while still being able to set and read properties from ASDisplayNode instances.
 */
...
- (CGRect)frame
{
  ...
  //#if DEBUG
  //  // Checking if the transform is identity is expensive, so disable when unnecessary. We have assertions on in
  //  Release, so DEBUG is the only way I know of. ASDisplayNodeAssert(CATransform3DIsIdentity(self.transform),
  //  @"-[ASDisplayNode frame] - self.transform must be identity in order to use the frame property.  (From Apple's
  //  UIView documentation: If the transform property is not the identity transform, the value of this property is
  ...
  BOOL isMainThread = ASDisplayNodeThreadIsMain();
  {
    ...
    if (!specialPropertiesHandling) {
      ...
      if (nodeLoaded && isMainThread) {
        ...
        // NOTE: Frame is only defined when transform is identity because we explicitly diverge from CALayer behavior
        // and define frame without transform.
        setToView = YES;
      } else {
...
- (void)setNeedsDisplay
{
  ...
  if (isRasterized) {
    ASPerformBlockOnMainThread(^{
      // The below operation must be performed on the main thread to ensure against an extremely rare deadlock, where a
      // parent node begins materializing the view / layer hierarchy (locking itself or a descendant) while this node
      // walks up the tree and requires locking that node to access .rasterizesSubtree. For this reason, this method
      // should be avoided when possible.  Use _hierarchyState & ASHierarchyStateRasterized.
      ASDisplayNodeAssertMainThread();
...
- (void)layoutIfNeeded
{
  ...
  id viewOrLayer = nil;
  {
    ...
    if (shouldApply == NO && loaded) {
      // The node is loaded but we're not on main.
      // We will call layoutIfNeeded on the view or layer when we apply the pending state. __layout will in turn be
      // called on us (see -[_ASDisplayLayer layoutSublayers]). We need to call it on main if the node is loaded to
      // support automatic subnode management. We can't release the lock before applying to pending state, or it may be
      // flushed before it can be applied.
...
- (void)setIsAccessibilityElement:(BOOL)isAccessibilityElement
{
  ...
  _setAccessibilityToViewAndProperty(
      _isAccessibilityElement, isAccessibilityElement, isAccessibilityElement, isAccessibilityElement);
}
...
- (void)setAccessibilityAttributedLabel:(NSAttributedString *)accessibilityAttributedLabel
{
  ...
  {
    _setAccessibilityToViewAndProperty(_accessibilityLabel,
                                       accessibilityAttributedLabel.string,
                                       accessibilityLabel,
                                       accessibilityAttributedLabel.string);
  }
}
...
- (void)setAccessibilityHint:(NSString *)accessibilityHint
{
  ...
  if (AS_AVAILABLE_IOS_TVOS(11, 11)) {
    ...
    _setAccessibilityToViewAndProperty(_accessibilityAttributedHint,
...
- (void)setAccessibilityAttributedHint:(NSAttributedString *)accessibilityAttributedHint
{
  ...
  {
    _setAccessibilityToViewAndProperty(
        _accessibilityHint, accessibilityAttributedHint.string, accessibilityHint, accessibilityAttributedHint.string);
  }
}
#endif
...
- (void)setAccessibilityValue:(NSString *)accessibilityValue
{
  ...
  if (AS_AVAILABLE_IOS_TVOS(11, 11)) {
    ...
    _setAccessibilityToViewAndProperty(_accessibilityAttributedValue,
...
- (void)setAccessibilityAttributedValue:(NSAttributedString *)accessibilityAttributedValue
{
  ...
  {
    _setAccessibilityToViewAndProperty(_accessibilityValue,
                                       accessibilityAttributedValue.string,
                                       accessibilityValue,
                                       accessibilityAttributedValue.string);
  }
}
...
- (void)setAccessibilityTraits:(UIAccessibilityTraits)accessibilityTraits
{
  _bridge_prologue_write;
  _setAccessibilityToViewAndProperty(
      _accessibilityTraits, accessibilityTraits, accessibilityTraits, accessibilityTraits);
}
...
- (void)setAccessibilityLanguage:(NSString *)accessibilityLanguage
{
  _bridge_prologue_write;
  _setAccessibilityToViewAndProperty(
      _accessibilityLanguage, accessibilityLanguage, accessibilityLanguage, accessibilityLanguage);
}
...
{
  _bridge_prologue_write;
  _setAccessibilityToViewAndProperty(_accessibilityElementsHidden,
...
- (void)setAccessibilityViewIsModal:(BOOL)accessibilityViewIsModal
{
  _bridge_prologue_write;
  _setAccessibilityToViewAndProperty(
      _accessibilityViewIsModal, accessibilityViewIsModal, accessibilityViewIsModal, accessibilityViewIsModal);
}
...
{
  _bridge_prologue_write;
  _setAccessibilityToViewAndProperty(_shouldGroupAccessibilityChildren,
...
- (void)setAccessibilityIdentifier:(NSString *)accessibilityIdentifier
{
  _bridge_prologue_write;
  _setAccessibilityToViewAndProperty(
      _accessibilityIdentifier, accessibilityIdentifier, accessibilityIdentifier, accessibilityIdentifier);
}

- (void)setAccessibilityNavigationStyle:(UIAccessibilityNavigationStyle)accessibilityNavigationStyle
{
  _bridge_prologue_write;
  _setAccessibilityToViewAndProperty(_accessibilityNavigationStyle,
...
{
  _bridge_prologue_write;
  _setAccessibilityToViewAndProperty(_accessibilityHeaderElements,
...
{
  _bridge_prologue_write;
  _setAccessibilityToViewAndProperty(_accessibilityActivationPoint,
...
{
  _bridge_prologue_write;
  _setToViewOrLayer(asyncdisplaykit_asyncTransactionContainer,
                    asyncTransactionContainer,
