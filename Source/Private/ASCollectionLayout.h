 ...
 *
 * @discussion To get the truth on the current state of the collection, call methods on the collection node or the data
 * source rather than the collection view because:
 ...
 *
 * @discussion For developers' convenience, the delegate is retained by this layout object, similar to UICollectionView
 * retains its UICollectionViewLayout object.
 *
 * @discussion For simplicity, the delegate is read-only. If a new layout delegate is needed, construct a new layout
 * object with that delegate and notify ASCollectionView about it. This ensures the underlying UICollectionView purges
 * its cache and properly loads the new layout.
 */
