                                                    ...
                                                    displayBlocks:(NSMutableArray *)displayBlocks
{
  ...
  // If this is the root container node, use a frame with a zero origin to draw into. If not, calculate the correct
  // frame using the node's position, transform and anchorPoint.
  if (self.rasterizesSubtree) {
    ...
    // Pretty hacky since full 3D transforms aren't actually supported, but attempt to compute the transformed frame of
    // this node so that we can composite it into approximately the right spot.
  ...
  asyncdisplaykit_async_transaction_operation_block_t displayBlock =
      [self _displayBlockWithAsynchronous:NO isCancelledBlock:isCancelledBlock rasterizing:YES];

  // We'll display something if there is a display block, clipping, translation and/or a background color.
  BOOL shouldDisplay =
      displayBlock || backgroundColor || CGPointEqualToPoint(CGPointZero, frame.origin) == NO || clipsToBounds;
  ...
  if (shouldDisplay) {
    dispatch_block_t pushAndDisplayBlock = ^{
      ...
      // If there is a display block, call it to get the image, then copy the image into the current context (which is
      // the rasterized container's backing store).
      if (displayBlock) {
                      ...
                      rasterizing:(BOOL)rasterizing
{
  ...
  // We always create a graphics context, unless a -display method is used, OR if we are a subnode drawing into a
  // rasterized parent.
  ...
  // Capture drawParameters from delegate on main thread, if this node is displaying itself rather than recursively
  // rasterizing.
  ...
  if (shouldBeginRasterizing) {
    ...
    // If [UIColor clearColor] or another semitransparent background color is used, include alpha channel when
    // rasterizing. Unlike CALayer drawing, we include the backgroundColor as a base during rasterization.
    ...
    displayBlock = ^id {
      ...
      // For -display methods, we don't have a context, and thus will not call the
      // _willDisplayNodeContentWithRenderingContext or _didDisplayNodeContentWithRenderingContext blocks. It's up to
      // the implementation of -display... to do what it needs.
      ...
      [self __didDisplayNodeContentWithRenderingContext:currentContext
                                                  image:&image
...
- (void)displayAsyncLayer:(_ASDisplayLayer *)asyncLayer asynchronously:(BOOL)asynchronously
{
  ...
  asyncdisplaykit_async_transaction_operation_block_t displayBlock =
      [self _displayBlockWithAsynchronous:asynchronously isCancelledBlock:isCancelledBlock rasterizing:NO];
  ...
  // This block is called back on the main thread after rendering at the completion of the current async transaction, or
  // immediately if !asynchronously
