...
- (BOOL)implementsLayoutMethod {
  ...
  return (_methodOverrides &
          (ASDisplayNodeMethodOverrideLayoutSpecThatFits | ASDisplayNodeMethodOverrideCalcLayoutThatFits |
           ASDisplayNodeMethodOverrideCalcSizeThatFits)) != 0 ||
         _layoutSpecBlock != nil;
}
...
- (ASLayout *)layoutThatFits:(ASSizeRange)constrainedSize parentSize:(CGSize)parentSize {
  ...
  if (_calculatedDisplayNodeLayout.isValid(constrainedSize, parentSize, version)) {
    ASDisplayNodeAssertNotNil(
        _calculatedDisplayNodeLayout.layout,
        @"-[ASDisplayNode layoutThatFits:parentSize:] _calculatedDisplayNodeLayout.layout should not be nil! %@", self);
    layout = _calculatedDisplayNodeLayout.layout;
  } else if (_pendingDisplayNodeLayout.isValid(constrainedSize, parentSize, version)) {
    ASDisplayNodeAssertNotNil(
        _pendingDisplayNodeLayout.layout,
        @"-[ASDisplayNode layoutThatFits:parentSize:] _pendingDisplayNodeLayout.layout should not be nil! %@", self);
    ...
    ASDisplayNodeAssertNotNil(
        layout, @"-[ASDisplayNode layoutThatFits:parentSize:] newly calculated layout should not be nil! %@", self);
  }
...
- (void)setPrimitiveTraitCollection:(ASPrimitiveTraitCollection)traitCollection {
  if (ASPrimitiveTraitCollectionIsEqualToASPrimitiveTraitCollection(traitCollection,
                                                                    _primitiveTraitCollection.load()) == NO) {
    _primitiveTraitCollection = traitCollection;
    ASDisplayNodeLogEvent(self, @"asyncTraitCollectionDidChange: %@",
...
- (void)_u_measureNodeWithBoundsIfNecessary:(CGRect)bounds {
  ...
  BOOL calculatedLayoutIsReusable =
      (_calculatedDisplayNodeLayout.isValid(_layoutVersion) &&
  ...
  as_log_verbose(ASLayoutLog(), "Node %@, bounds size %@, calculatedSize %@, calculatedIsDirty %d", self,
                 NSStringFromCGSize(boundsSizeForLayout), NSStringFromCGSize(_calculatedDisplayNodeLayout.layout.size),
...
#define layoutSizeDifferentFromBounds !CGSizeEqualToSize(nextLayout.layout.size, boundsSizeForLayout)
  ...
  if (nextLayout.layout == nil) {
    ...
    as_log_verbose(ASLayoutLog(), "Pending layout size %@ doesn't match bounds size.",
                   NSStringFromCGSize(nextLayout.layout.size));
  } else {
  ...
  if (nextLayout.requestedLayoutFromAbove == NO &&
    ...
    // Update the layout's version here because _u_setNeedsLayoutFromAbove calls __setNeedsLayout which in turn
    // increases _layoutVersion Failing to do this will cause the layout to be invalid immediately
...
- (void)transitionLayoutWithSizeRange:(ASSizeRange)constrainedSize
  ...
  as_log_debug(ASLayoutLog(), "Transition layout for %@ sizeRange %@ anim %d asyncMeasure %d", self,
               NSStringFromASSizeRange(constrainedSize), animated, shouldMeasureAsync);
  ...
  ASDisplayNodePerformBlockOnEverySubnode(self, NO, ^(ASDisplayNode *_Nonnull node) {
    ASDisplayNodeAssert(node->_transitionID == ASLayoutElementContextInvalidTransitionID,
  ...
  void (^transitionBlock)(void) = ^{
    ...
    ASPerformBlockOnMainThread(^{
      ...
      _ASTransitionContext *pendingLayoutTransitionContext;
      {
        ...
        const auto pendingLayout =
            ASDisplayNodeLayout(newLayout, constrainedSize, constrainedSize.max, newLayoutVersion);
        ...
        _pendingLayoutTransitionContext = pendingLayoutTransitionContext =
            [[_ASTransitionContext alloc] initWithAnimation:animated
...
- (void)animateLayoutTransition:(id<ASContextTransitioning>)context {
  ...
  for (ASDisplayNode *insertedSubnode in insertedSubnodes) {
    [insertedSubnodeContexts addObject:[_ASAnimatedTransitionContext contextForNode:insertedSubnode
                                                                              alpha:insertedSubnode.alpha]];
  }
  for (ASDisplayNode *removedSubnode in removedSubnodes) {
    [removedSubnodeContexts addObject:[_ASAnimatedTransitionContext contextForNode:removedSubnode
                                                                             alpha:removedSubnode.alpha]];
  }
...
- (void)_completeLayoutTransition:(ASLayoutTransition *)layoutTransition {
  ...
  if (ASDisplayNodeThreadIsMain() || layoutTransition.isSynchronous == NO) {
    // Committing the layout transition will result in subnode insertions and removals, both of which must be called
    // without the lock held
    // TODO: Disabled due to PR: https://github.com/TextureGroup/Texture/pull/1204
    // ASAssertUnlocked(__instanceLock__);
    [layoutTransition commitTransition];
  } else {
    // Subnode insertions and removals need to happen always on the main thread if at least one subnode is already
    // loaded
    ASPerformBlockOnMainThread(^{
      [layoutTransition commitTransition];
    });
...
- (void)_assertSubnodeState {
  ...
  NSArray<ASDisplayNode *> *layoutNodes =
      ASArrayByFlatMapping(sublayouts, ASLayout * layout, (ASDisplayNode *)layout.layoutElement);
  NSIndexSet *insertions, *deletions;
  [_subnodes asdk_diffWithArray:layoutNodes insertions:&insertions deletions:&deletions];
  if (insertions.count > 0) {
    NSLog(@"Warning: node's layout includes subnode that has not been added: node = %@, subnodes = %@, subnodes in "
          @"layout = %@",
          self, _subnodes, layoutNodes);
  }
...
- (void)_pendingLayoutTransitionDidComplete {
  // This assertion introduces a breaking behavior for nodes that has ASM enabled but also manually manage some
  // subnodes. Let's gate it behind YOGA flag.
#if YOGA
  ...
  // First measurement is guaranteed to be before the node is onscreen, so we can create the image async. but still have
  // it appear sync.
