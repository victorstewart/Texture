...
- (void)layout {
  ...
  if ([ASControlNode enableHitTestDebug]) {
    // Construct hitTestDebug highlight overlay frame indicating tappable area of a node, which can be restricted by two
    // things:

    // (1) Any parent's tapable area (its own bounds + hitTestSlop) may restrict the desired tappable area expansion
    // using hitTestSlop of a child as UIKit event delivery (hitTest:) will not search sub-hierarchies if one of our
    // parents does not return YES for pointInside:. To circumvent this restriction, a developer will need to set /
    // adjust the hitTestSlop on the limiting parent. This is indicated in the overlay by a dark GREEN edge. This is an
    // ACTUAL restriction.

    // (2) Any parent's .clipToBounds. If a parent is clipping, we cannot show the overlay outside that area
    // (although it still respond to touch). To indicate that the overlay cannot accurately display the true tappable
    // area, the overlay will have an ORANGE edge. This is a VISUALIZATION restriction.
    ...
    // FIXED: Stop climbing hierarchy if UIScrollView is encountered (its offset bounds origin may make it seem like our
    // events will be clipped when scrolling will actually reveal them (because this process will not re-run due to
    // scrolling))
    while (intersectSuperlayer && ![intersectSuperlayer.delegate respondsToSelector:@selector(contentOffset)]) {
      ...
      CGRect intersectRectInParentCoordinates = [intersectSuperlayer convertRect:intersectRect
    ...
    if (clippedEdges == UIRectEdgeNone) {
      ...
      debugOverlay.image = [debugHighlightImage resizableImageWithCapInsets:edgeInsets
...
- (void)layoutToFitAllBarsExcept:(NSInteger)barsToClip {
  ...
  for (_ASRangeDebugBarView *barView in [_rangeControllerViews copy]) {
    ...
    ASInterfaceState interfaceState =
        [barView.rangeController.dataSource interfaceStateForRangeController:barView.rangeController];
               ...
               interfaceState:(ASInterfaceState)interfaceState;
{
  ...
  if (!((displayTuningParameters.leadingBufferScreenfuls + displayTuningParameters.trailingBufferScreenfuls) == 0)) {
    leadingDisplayTuningRatio =
        displayTuningParameters.leadingBufferScreenfuls /
        (displayTuningParameters.leadingBufferScreenfuls + displayTuningParameters.trailingBufferScreenfuls);
  }
  if (!((preloadTuningParameters.leadingBufferScreenfuls + preloadTuningParameters.trailingBufferScreenfuls) == 0)) {
    leadingPreloadTuningRatio =
        preloadTuningParameters.leadingBufferScreenfuls /
        (preloadTuningParameters.leadingBufferScreenfuls + preloadTuningParameters.trailingBufferScreenfuls);
  }
...
- (_ASRangeDebugBarView *)barViewForRangeController:(ASRangeController *)controller {
  ...
  for (_ASRangeDebugBarView *rangeView in [[_rangeControllerViews reverseObjectEnumerator] allObjects]) {
    ...
    ASInterfaceState interfaceState =
        [rangeView.rangeController.dataSource interfaceStateForRangeController:rangeView.rangeController];
...
- (void)layoutSubviews {
  ...
  [UIView animateWithDuration:animate ? 0.3 : 0.0
                   ...
                   animations:^{
                     _visibleRect.frame =
                         CGRectMake(HORIZONTAL_INSET + visiblePoint, rect.origin.y, visibleDimension, subCellHeight);
                     _displayRect.frame =
                         CGRectMake(HORIZONTAL_INSET + displayPoint, rect.origin.y, displayDimension, subCellHeight);
                     _preloadRect.frame =
                         CGRectMake(HORIZONTAL_INSET + preloadPoint, rect.origin.y, preloadDimension, subCellHeight);
                   }
                   completion:^(BOOL finished){
                   }];
