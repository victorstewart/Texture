...
- (ASInterfaceState)interfaceState
{
  ...
  if (__ApplicationState == UIApplicationStateBackground) {
    // If the app is background, pretend to be invisible so that we inform each cell it is no longer being viewed by the
    // user
...
- (void)_updateVisibleNodeIndexPaths
{
  ...
  if (visibleElements.count ==
      0) {  // if we don't have any visibleNodes currently (scrolled before or after content)...
    // Verify the actual state by checking the layout with a "VisibleOnly" range.
    // This allows us to avoid thrashing through -didExitVisibleState in the case of -reloadData, since that generates
    // didEndDisplayingCell calls. Those didEndDisplayingCell calls result in items being removed from the
    // visibleElements returned by the _dataSource, even though the layout remains correct.
    visibleElements = [_layoutController elementsForScrolling:scrollDirection
                                                    rangeMode:ASLayoutRangeModeVisibleOnly
  ...
  // If the range mode is explicitly set via updateCurrentRangeWithMode:, we'll preserve that for at least one update
  // cycle. Once the user has scrolled and the range is visible, we'll always resume managing the range mode
  // automatically.
  if ((updateRangeMode && ASInterfaceStateIncludesVisible(selfInterfaceState)) ||
      [[self class] isFirstRangeUpdateForRangeMode:rangeMode]) {
  ...
  // Preload can express the ultra-low-memory state with 0, 0 returned for its tuningParameters above, and will match
  // Visible. However, in this rangeMode, Display is not supposed to contain *any* paths -- not even the visible bounds.
  // TuningParameters can't express this.
  ...
  BOOL equalDisplayVisible =
      (ASRangeTuningParametersEqualToRangeTuningParameters(parametersDisplay, ASRangeTuningParametersZero) &&
       emptyDisplayRange == NO);

  // Check if both Display and Preload are unique. If they are, we load them with a single fetch from the layout
  // controller for performance.
  BOOL optimizedLoadingOfBothRanges =
      (equalDisplayPreload == NO && equalDisplayVisible == NO && emptyDisplayRange == NO);
  ...
  if (optimizedLoadingOfBothRanges) {
    ...
    if (emptyDisplayRange == YES) {
      ...
      displayElements = [_layoutController elementsForScrolling:scrollDirection
                                                      rangeMode:rangeMode
                                                      rangeType:ASLayoutRangeTypeDisplay
                                                            map:map];
    }

    BOOL equalPreloadVisible =
        ASRangeTuningParametersEqualToRangeTuningParameters(parametersPreload, ASRangeTuningParametersZero);
    if (equalDisplayPreload == YES) {
      preloadElements = displayElements;
    } else if (equalPreloadVisible == YES) {
      preloadElements = visibleElements;
    } else {
      preloadElements = [_layoutController elementsForScrolling:scrollDirection
                                                      rangeMode:rangeMode
                                                      rangeType:ASLayoutRangeTypePreload
                                                            map:map];
    }
  ...
  NSSet<NSIndexPath *> *visibleIndexPaths =
      ASSetByFlatMapping(visibleElements, ASCollectionElement * element, [map indexPathForElementIfCell:element]);
  NSSet<NSIndexPath *> *displayIndexPaths =
      ASSetByFlatMapping(displayElements, ASCollectionElement * element, [map indexPathForElementIfCell:element]);
  NSSet<NSIndexPath *> *preloadIndexPaths =
      ASSetByFlatMapping(preloadElements, ASCollectionElement * element, [map indexPathForElementIfCell:element]);
  ...
  for (NSIndexPath *indexPath in allIndexPaths) {
    ...
    if (ASInterfaceStateIncludesVisible(selfInterfaceState)) {
      ...
      // If selfInterfaceState isn't visible, then visibleIndexPaths represents what /will/ be immediately visible at
      // the instant we come onscreen.  So, preload and display all of those things, but don't waste resources
      // preloading yet. We handle this as a separate case to minimize set operations for offscreen preloading,
      // including containsObject:.
      ...
      if ([allCurrentIndexPaths containsObject:indexPath]) {
        ...
        // our overall container object is itself not visible yet.  The moment it becomes visible, we will run the
        // condition above
        ...
        if (rangeMode != ASLayoutRangeModeLowMemory) {
          ...
          // In that case we'll just set it back to MeasureLayout.  Only set Display | Preload if in
          // allCurrentIndexPaths.
          interfaceState |= ASInterfaceStateDisplay;
        }
      }
    }
    ...
    if (node != nil) {
      ASDisplayNodeAssert(
          node.hierarchyState & ASHierarchyStateRangeManaged,
...
+ (void)didReceiveMemoryWarning:(NSNotification *)notification
{
  ...
  for (ASRangeController *rangeController in allRangeControllers) {
    ...
    [rangeController
        updateCurrentRangeWithMode:isDisplay ? ASLayoutRangeModeVisibleOnly : __rangeModeForMemoryWarnings];
...
+ (void)didEnterBackground:(NSNotification *)notification
{
  ...
  for (ASRangeController *rangeController in allRangeControllers) {
    ...
    // avoided when the app is resumed.  Non-visible controllers can be more aggressively culled to the LowMemory state
  ...
  // Because -interfaceState checks __ApplicationState and always clears the "visible" bit if Backgrounded, we must set
  // this after updating the range mode.
  __ApplicationState = UIApplicationStateBackground;
  for (ASRangeController *rangeController in allRangeControllers) {
    // Trigger a range update immediately, as we may not be allowed by the system to run the update block scheduled by
    // changing range mode. There's no need to call needs update as updateCurrentRangeWithMode sets this if necessary.
    [rangeController updateIfNeeded];
  }
...
- (NSString *)descriptionWithIndexPaths:(NSArray<NSIndexPath *> *)indexPaths
{
  ...
  for (NSIndexPath *indexPath in indexPaths) {
    ASDisplayNode *node =
        [[_dataSource elementMapForRangeController:self] elementForItemAtIndexPath:indexPath].nodeIfAllocated;
