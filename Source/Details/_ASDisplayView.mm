...
+ (void)initialize {
  ...
  IMP staticInitialize = imp_implementationWithBlock(^(_ASDisplayView *view) {
    ASDisplayNodeAssert(view.class == initializeSelf,
                        @"View class %@ does not have a matching _staticInitialize method; check to ensure [super "
...
- (NSString *)description {
  ...
  if (node != nil) {
    ...
    [description replaceOccurrencesOfString:@"_ASDisplay"
    ...
    if (layerDescriptionRange.location != NSNotFound) {
      ...
      // Our regex will grab the closing angle bracket and I'm not clever enough to come up with a better one, so re-add
      // it if needed.
...
- (void)willMoveToSuperview:(UIView *)newSuperview {
  ...
  if (newSuperview) {
    ...
    ASDisplayNodeAssert(!supernode.isLayerBacked,
                        @"Shouldn't be possible for _ASDisplayView's supernode to be layer-backed.");
    ...
    if (supernode) {
      if (supernodeLoaded) {
        if (supernode.layerBacked) {
          // See comment in -didMoveToSuperview.  This case should be avoided, but is possible with app-level coding
          // errors.
          needsSupernodeUpdate = (supernode.layer != newSuperview.layer);
        } else {
          // If we have a supernode, compensate for users directly messing with views by hitching up to any new
          // supernode.
...
- (void)didMoveToSuperview {
  ...
  if (superview == nil) {
    // Clearing keepalive_node may cause deallocation of the node.  In this case, __exitHierarchy may not have an
    // opportunity (e.g. _node will be cleared by the time -didMoveToWindow occurs after this) to clear the Visible
    // interfaceState, which we need to do before deallocation to meet an API guarantee.
    if (node.inHierarchy) {
      [node __exitHierarchy];
    }
...
#if DEBUG
  // This is only to help detect issues when a root-of-view-controller node is reused separately from its view
  // controller. Avoid overhead in release.
  if (superview && node.viewControllerRoot) {
    UIViewController *vc = [node closestViewController];

    ASDisplayNodeAssert(
        vc != nil && [vc isKindOfClass:[ASViewController class]] && ((ASViewController *)vc).node == node,
        @"This node was once used as a view controller's node. You should not reuse it without its view controller.");
  }
#endif
  ...
  if (supernode) {
    ...
    if (superview) {
      ...
      if (supernodeLoaded) {
        if (supernode.layerBacked) {
          // As asserted at the top, this shouldn't be possible, but in production with assertions disabled it can
          // happen. We try to make such code behave as well as feasible because it's not that hard of an error to make
          // if some deep child node of a layer-backed node happens to be view-backed, but it is not supported and
          // should be avoided.
      ...
      // In the latter case, we don't want to disassemble the node hierarchy because all views are intentionally being
      // destroyed.
...
- (void)setContentMode:(UIViewContentMode)contentMode {
  ASDisplayNodeAssert(contentMode != UIViewContentModeRedraw,
                      @"Don't do this. Use needsDisplayOnBoundsChange instead.");

  // Do our own mapping so as not to call super and muck up needsDisplayOnBoundsChange. If we're in a production build,
  // fall back to resize if we see redraw
  self.layer.contentsGravity = (contentMode != UIViewContentModeRedraw)
                                   ? ASDisplayNodeCAContentsGravityFromUIContentMode(contentMode)
                                   : kCAGravityResize;
}
...
- (id)forwardingTargetForSelector:(SEL)aSelector {
  // Ideally, we would implement -targetForAction:withSender: and simply return the node where we don't respond
  // personally. Unfortunately UIResponder's default implementation of -targetForAction:withSender: doesn't follow its
  // own documentation. It doesn't call -targetForAction:withSender: up the responder chain when
  // -canPerformAction:withSender: fails, but instead merely calls -canPerformAction:withSender: on itself and then up
  // the chain. rdar://20111500. Consequently, to forward responder-chain actions to our node, we override
  // -canPerformAction:withSender: (used by the chain) to indicate support for responder chain-driven actions that our
  // node supports, and then provide the node as a forwarding target here.
  ASDisplayNode *node = _asyncdisplaykit_node;  // Create strong reference to weak ivar.
  return node;
}
