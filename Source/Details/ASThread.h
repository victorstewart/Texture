...
#define ASLocked(nsLocking, expr) \
  ({                              \
    ASLockScope(nsLocking);       \
    expr;                         \
  })

/// Faster self-version.
#define ASLockedSelf(expr) \
  ({                       \
    ASLockScopeSelf();     \
    expr;                  \
  })

#define ASLockedSelfCompareAssign(lvalue, newValue) ASLockedSelf(ASCompareAssign(lvalue, newValue))

#define ASLockedSelfCompareAssignObjects(lvalue, newValue) ASLockedSelf(ASCompareAssignObjects(lvalue, newValue))

#define ASLockedSelfCompareAssignCustom(lvalue, newValue, isequal) \
  ASLockedSelf(ASCompareAssignCustom(lvalue, newValue, isequal))

#define ASLockedSelfCompareAssignCopy(lvalue, obj) ASLockedSelf(ASCompareAssignCopy(lvalue, obj))
