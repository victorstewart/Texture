...
/**
 * ASDisplayNode loaded callback block. This block is called BEFORE the -didLoad method and is always called on the main
 * thread.
 */
...
@interface ASDisplayNode : NSObject <ASLocking> {
 @public
  /**
   * The _context ivar is unused by Texture, but provided to enable advanced clients to make powerful extensions to base
   * class functionality. For example, _context can be used to implement category methods on ASDisplayNode that add
   * functionality to all node subclass types. Code demonstrating this technique can be found in the
   * CatDealsCollectionView example.
   */
  void *_context;
}
...
/**
 * @abstract Add a block of work to be performed on the main thread when the node's view or layer is loaded. Thread
 * safe.
 * @warning Be careful not to retain self in `body`. Change the block parameter list to `^(MYCustomNode *self) {}` if
 * you want to shadow self (e.g. if calling this during `init`).
 *
 ...
 * If the node is removed from a visible hierarchy and then re-added, it will automatically trigger a new asynchronous
 * display, as long as displaySuspended is not set. If the node remains in the hierarchy throughout, -setNeedsDisplay is
 * required to trigger a new asynchronous display.
 *
 ...
 *
 * Flattened layouts use less memory and are faster to lookup. On the other hand, unflattened layouts are useful for
 * debugging because they preserve original information.
 *
 * Defaults to NO.
 */
 ...
 *
 * - NOTE: After the view or layer is created, the properties pass through to the view or layer directly and must be
...
/**
 * Marks the view as needing display. Convenience for use whether the view / layer is loaded or not. Safe to call from a
 * background thread.
 */
- (void)setNeedsDisplay;

/**
 * Marks the node as needing layout. Convenience for use whether the view / layer is loaded or not. Safe to call from a
 * background thread.
 *
 * If the node determines its own desired layout size will change in the next layout pass, it will propagate this
 * information up the tree so its parents can have a chance to consider and apply if necessary the new size onto the
 * node.
 *
...
/**
 * Performs a layout pass on the node. Convenience for use whether the view / layer is loaded or not. Safe to call from
 * a background thread.
 */
- (void)layoutIfNeeded;
 ...
 */
@property ASCornerRoundingType
    cornerRoundingType;  // default=ASCornerRoundingTypeDefaultSlowCALayer .cornerRadius (offscreen rendering)
 ...
 *
 * If your layout depends on this property, you should probably enable automaticallyRelayoutOnLayoutMarginsChanges to
 * ensure that the layout gets automatically updated when the value of this property changes. Or you can override
 * layoutMarginsDidChange and make all the necessary updates manually.
 */
@property UIEdgeInsets layoutMargins;
@property BOOL preservesSuperviewLayoutMargins;  // default is NO - set to enable pass-through or cascading behavior of
                                                 // margins from this view’s parent to its children
 ...
 * that the layout gets automatically updated when the value of this property changes. Or you can override
 * safeAreaInsetsDidChange and make all the necessary updates manually.
 */
 ...
 *
 * @warning Subclasses must not override this; it caches results from -calculateLayoutThatFits:.  Calling this method
 * may be expensive if result is not cached.
 *
...
typedef NS_ENUM(NSInteger, ASLayoutEngineType) { ASLayoutEngineTypeLayoutSpec, ASLayoutEngineTypeYoga };
...
/**
 * @discussion A place to perform your animation. New nodes have been inserted here. You can also use this time to
 ...
 *
 * @param animated Animation is optional, but will still proceed through your `animateLayoutTransition` implementation
 * with `isAnimated == NO`.
 ...
 *
 * @discussion If the passed constrainedSize is the the same as the node's current constrained size, this method is
 * noop. If passed YES to shouldMeasureAsync it's guaranteed that measurement is happening on a background thread,
 * otherwise measaurement will happen on the thread that the method was called on. The measurementCompletion callback is
 * always called on the main thread right after the measurement and before -animateLayoutTransition:.
 *
...
/**
 * @abstract Invalidates the layout and begins a relayout of the node with the current `constrainedSize`. Must be called
 * on main thread.
 *
