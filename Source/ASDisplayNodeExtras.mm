...
void _ASSetDebugNames(Class _Nonnull owningClass, NSString *_Nonnull names, ASDisplayNode *_Nullable object, ...) {
  ...
  for (ASDisplayNode *node = object; node != nil; node = va_arg(args, id), i++) {
    ...
    [symbolName replaceOccurrencesOfString:@"self."
                                withString:@""
                                   options:NSAnchoredSearch
                                     range:NSMakeRange(0, symbolName.length)];
    [symbolName replaceOccurrencesOfString:@"_"
                                withString:@""
                                   options:NSAnchoredSearch
                                     range:NSMakeRange(0, symbolName.length)];
...
void ASDisplayNodePerformBlockOnEveryNode(CALayer *_Nullable layer,
  ...
  if (traverseSublayers && layer && node.rasterizesSubtree == NO) {
    ...
    /// See:
    /// http://stackoverflow.com/questions/14854480/collection-calayerarray-0x1ed8faa0-was-mutated-while-being-enumerated
