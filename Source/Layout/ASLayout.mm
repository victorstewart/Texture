  ...
  while (!queue.empty()) {
    ...
    if (ASLayoutIsDisplayNodeType(layout)) {
      if (sublayoutsCount > 0 || CGPointEqualToPoint(ASCeilPointValues(absolutePosition), layout.position) == NO) {
        // Only create a new layout if the existing one can't be reused, which means it has either some sublayouts or an
      ...
      // Fast-reverse-enumerate the sublayouts array by copying it into a C-array and push_front'ing each into the
      // queue.
