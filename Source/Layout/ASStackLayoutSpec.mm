...
- (void)setAlignItems:(ASStackLayoutAlignItems)alignItems
{
  ...
  ASDisplayNodeAssert(_verticalAlignment == ASVerticalAlignmentNone,
                      @"Cannot set this property directly because verticalAlignment is being used");
  _alignItems = alignItems;
}
...
- (ASLayout *)calculateLayoutThatFits:(ASSizeRange)constrainedSize
{
  ...
  as_activity_scope_verbose(
      as_activity_create("Calculate stack layout", AS_ACTIVITY_CURRENT, OS_ACTIVITY_FLAG_DEFAULT));
  ...
  const ASStackLayoutSpecStyle style = {.direction = _direction,
                                        .spacing = _spacing,
  ...
  const auto unpositionedLayout =
      ASStackUnpositionedLayout::compute(stackChildren, style, constrainedSize, _concurrent);
