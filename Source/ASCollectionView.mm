...
@interface ASCollectionView () <ASRangeControllerDataSource,
                                ASRangeControllerDelegate,
                                ASDataControllerSource,
                                ASCellNodeInteractionDelegate,
                                ASDelegateProxyInterceptor,
                                ASBatchFetchingScrollView,
   ...
   * Rationale:
   * In `reloadData`, a collection view invalidates its data and marks itself as needing reload, and waits until
   `layoutSubviews` to requery its data source.
   * This can lead to data inconsistency problems.
   * Say you have an empty collection view. You call `reloadData`, then immediately insert an item into your data source
   and call `insertItemsAtIndexPaths:[0,0]`.
   * You will get an assertion failure saying `Invalid number of items in section 0.
   * The number of items after the update (1) must be equal to the number of items before the update (1) plus or minus
   the items added and removed (1 added, 0 removed).`
  ...
  struct {
    ...
    unsigned int modelIdentifierMethods : 1;  // if both modelIdentifierForElementAtIndexPath and
                                              // indexPathForElementWithModelIdentifier are implemented
...
- (instancetype)_initWithFrame:(CGRect)frame
          collectionViewLayout:(UICollectionViewLayout *)layout
             layoutFacilitator:(id<ASCollectionViewLayoutFacilitatorProtocol>)layoutFacilitator
                    owningNode:(ASCollectionNode *)owningNode
...
- (void)dealloc {
  ...
  // Sometimes the UIKit classes can call back to their delegate even during deallocation, due to animation completion
  // blocks etc.
  _isDeallocating = YES;
...
- (void)setDataSource:(id<UICollectionViewDataSource>)dataSource {
  // UIKit can internally generate a call to this method upon changing the asyncDataSource; only assert for non-nil. We
  // also allow this when we're doing interop.
  ASDisplayNodeAssert(_asyncDelegateFlags.interop || dataSource == nil,
                      @"ASCollectionView uses asyncDataSource, not UICollectionView's dataSource property.");
}

- (void)setDelegate:(id<UICollectionViewDelegate>)delegate {
  // Our UIScrollView superclass sets its delegate to nil on dealloc. Only assert if we get a non-nil value here. We
  // also allow this when we're doing interop.
  ASDisplayNodeAssert(_asyncDelegateFlags.interop || delegate == nil,
                      @"ASCollectionView uses asyncDelegate, not UICollectionView's delegate property.");
}
...
- (void)setAsyncDataSource:(id<ASCollectionDataSource>)asyncDataSource {
  ...
  // will return as nil (ARC magic) even though the _proxyDataSource still exists. It's really important to hold a
  // strong reference to the old dataSource in this case because calls to ASCollectionViewProxy will start failing and
  // cause crashes.
  ...
  if (asyncDataSource == nil) {
    ...
    _asyncDataSourceFlags.collectionViewNodeForItem =
        [_asyncDataSource respondsToSelector:@selector(collectionView:nodeForItemAtIndexPath:)];
    ...
    _asyncDataSourceFlags.numberOfSectionsInCollectionView =
        [_asyncDataSource respondsToSelector:@selector(numberOfSectionsInCollectionView:)];
    _asyncDataSourceFlags.collectionViewNumberOfItemsInSection =
        [_asyncDataSource respondsToSelector:@selector(collectionView:numberOfItemsInSection:)];
    _asyncDataSourceFlags.collectionViewNodeForSupplementaryElement =
        [_asyncDataSource respondsToSelector:@selector(collectionView:nodeForSupplementaryElementOfKind:atIndexPath:)];

    _asyncDataSourceFlags.collectionNodeNodeForItem =
        [_asyncDataSource respondsToSelector:@selector(collectionNode:nodeForItemAtIndexPath:)];
    _asyncDataSourceFlags.collectionNodeNodeBlockForItem =
        [_asyncDataSource respondsToSelector:@selector(collectionNode:nodeBlockForItemAtIndexPath:)];
    _asyncDataSourceFlags.numberOfSectionsInCollectionNode =
        [_asyncDataSource respondsToSelector:@selector(numberOfSectionsInCollectionNode:)];
    _asyncDataSourceFlags.collectionNodeNumberOfItemsInSection =
        [_asyncDataSource respondsToSelector:@selector(collectionNode:numberOfItemsInSection:)];
    _asyncDataSourceFlags.collectionNodeContextForSection =
        [_asyncDataSource respondsToSelector:@selector(collectionNode:contextForSection:)];
    _asyncDataSourceFlags.collectionNodeNodeForSupplementaryElement =
        [_asyncDataSource respondsToSelector:@selector(collectionNode:nodeForSupplementaryElementOfKind:atIndexPath:)];
    _asyncDataSourceFlags.collectionNodeNodeBlockForSupplementaryElement = [_asyncDataSource
        respondsToSelector:@selector(collectionNode:nodeBlockForSupplementaryElementOfKind:atIndexPath:)];
    _asyncDataSourceFlags.collectionNodeSupplementaryElementKindsInSection =
        [_asyncDataSource respondsToSelector:@selector(collectionNode:supplementaryElementKindsInSection:)];
    _asyncDataSourceFlags.nodeModelForItem = [_asyncDataSource respondsToSelector:@selector(collectionNode:
                                                                                      nodeModelForItemAtIndexPath:)];
    _asyncDataSourceFlags.collectionNodeCanMoveItem = [_asyncDataSource respondsToSelector:@selector(collectionNode:
                                                                                                canMoveItemWithNode:)];
    _asyncDataSourceFlags.collectionNodeMoveItem =
        [_asyncDataSource respondsToSelector:@selector(collectionNode:moveItemAtIndexPath:toIndexPath:)];
    ...
    if (_asyncDataSourceFlags.interop) {
      ...
      _asyncDataSourceFlags.interopAlwaysDequeue =
          [[interopDataSource class] respondsToSelector:@selector(dequeuesCellsForNodeBackedItems)] &&
          [[interopDataSource class] dequeuesCellsForNodeBackedItems];
      _asyncDataSourceFlags.interopViewForSupplementaryElement = [interopDataSource
          respondsToSelector:@selector(collectionView:viewForSupplementaryElementOfKind:atIndexPath:)];
    }

    _asyncDataSourceFlags.modelIdentifierMethods =
        [_asyncDataSource respondsToSelector:@selector(modelIdentifierForElementAtIndexPath:inNode:)] &&
        [_asyncDataSource respondsToSelector:@selector(indexPathForElementWithModelIdentifier:inNode:)];

    ASDisplayNodeAssert(_asyncDataSourceFlags.collectionNodeNumberOfItemsInSection ||
                            _asyncDataSourceFlags.collectionViewNumberOfItemsInSection,
                        @"Data source must implement collectionNode:numberOfItemsInSection:");
    ASDisplayNodeAssert(
        _asyncDataSourceFlags.collectionNodeNodeBlockForItem || _asyncDataSourceFlags.collectionNodeNodeForItem ||
            _asyncDataSourceFlags.collectionViewNodeBlockForItem || _asyncDataSourceFlags.collectionViewNodeForItem,
        @"Data source must implement collectionNode:nodeBlockForItemAtIndexPath: or "
...
- (void)setAsyncDelegate:(id<ASCollectionDelegate>)asyncDelegate {
  ...
  // will return as nil (ARC magic) even though the _proxyDataSource still exists. It's really important to hold a
  // strong reference to the old delegate in this case because calls to ASCollectionViewProxy will start failing and
  // cause crashes.
  ...
  if (asyncDelegate == nil) {
    ...
    _asyncDelegateFlags.scrollViewWillEndDragging =
        [_asyncDelegate respondsToSelector:@selector(scrollViewWillEndDragging:withVelocity:targetContentOffset:)];
    _asyncDelegateFlags.scrollViewDidEndDecelerating =
        [_asyncDelegate respondsToSelector:@selector(scrollViewDidEndDecelerating:)];
    _asyncDelegateFlags.scrollViewWillBeginDragging =
        [_asyncDelegate respondsToSelector:@selector(scrollViewWillBeginDragging:)];
    _asyncDelegateFlags.scrollViewDidEndDragging =
        [_asyncDelegate respondsToSelector:@selector(scrollViewDidEndDragging:willDecelerate:)];
    _asyncDelegateFlags.collectionViewWillDisplayNodeForItem =
        [_asyncDelegate respondsToSelector:@selector(collectionView:willDisplayNode:forItemAtIndexPath:)];
    if (_asyncDelegateFlags.collectionViewWillDisplayNodeForItem == NO) {
      _asyncDelegateFlags.collectionViewWillDisplayNodeForItemDeprecated =
          [_asyncDelegate respondsToSelector:@selector(collectionView:willDisplayNodeForItemAtIndexPath:)];
    }
    _asyncDelegateFlags.collectionViewDidEndDisplayingNodeForItem =
        [_asyncDelegate respondsToSelector:@selector(collectionView:didEndDisplayingNode:forItemAtIndexPath:)];
    _asyncDelegateFlags.collectionViewWillBeginBatchFetch =
        [_asyncDelegate respondsToSelector:@selector(collectionView:willBeginBatchFetchWithContext:)];
    _asyncDelegateFlags.shouldBatchFetchForCollectionView =
        [_asyncDelegate respondsToSelector:@selector(shouldBatchFetchForCollectionView:)];
    _asyncDelegateFlags.collectionViewShouldSelectItem =
        [_asyncDelegate respondsToSelector:@selector(collectionView:shouldSelectItemAtIndexPath:)];
    _asyncDelegateFlags.collectionViewDidSelectItem =
        [_asyncDelegate respondsToSelector:@selector(collectionView:didSelectItemAtIndexPath:)];
    _asyncDelegateFlags.collectionViewShouldDeselectItem =
        [_asyncDelegate respondsToSelector:@selector(collectionView:shouldDeselectItemAtIndexPath:)];
    _asyncDelegateFlags.collectionViewDidDeselectItem =
        [_asyncDelegate respondsToSelector:@selector(collectionView:didDeselectItemAtIndexPath:)];
    _asyncDelegateFlags.collectionViewShouldHighlightItem =
    ...
    _asyncDelegateFlags.collectionViewDidHighlightItem =
        [_asyncDelegate respondsToSelector:@selector(collectionView:didHighlightItemAtIndexPath:)];
    _asyncDelegateFlags.collectionViewDidUnhighlightItem =
        [_asyncDelegate respondsToSelector:@selector(collectionView:didUnhighlightItemAtIndexPath:)];
    _asyncDelegateFlags.collectionViewShouldShowMenuForItem =
        [_asyncDelegate respondsToSelector:@selector(collectionView:shouldShowMenuForItemAtIndexPath:)];
    _asyncDelegateFlags.collectionViewCanPerformActionForItem =
        [_asyncDelegate respondsToSelector:@selector(collectionView:canPerformAction:forItemAtIndexPath:withSender:)];
    _asyncDelegateFlags.collectionViewPerformActionForItem =
        [_asyncDelegate respondsToSelector:@selector(collectionView:performAction:forItemAtIndexPath:withSender:)];
    _asyncDelegateFlags.collectionNodeWillDisplayItem =
        [_asyncDelegate respondsToSelector:@selector(collectionNode:willDisplayItemWithNode:)];
    _asyncDelegateFlags.collectionNodeDidEndDisplayingItem =
        [_asyncDelegate respondsToSelector:@selector(collectionNode:didEndDisplayingItemWithNode:)];
    _asyncDelegateFlags.collectionNodeWillBeginBatchFetch =
        [_asyncDelegate respondsToSelector:@selector(collectionNode:willBeginBatchFetchWithContext:)];
    _asyncDelegateFlags.shouldBatchFetchForCollectionNode =
        [_asyncDelegate respondsToSelector:@selector(shouldBatchFetchForCollectionNode:)];
    _asyncDelegateFlags.collectionNodeShouldSelectItem =
        [_asyncDelegate respondsToSelector:@selector(collectionNode:shouldSelectItemAtIndexPath:)];
    _asyncDelegateFlags.collectionNodeDidSelectItem =
        [_asyncDelegate respondsToSelector:@selector(collectionNode:didSelectItemAtIndexPath:)];
    _asyncDelegateFlags.collectionNodeShouldDeselectItem =
        [_asyncDelegate respondsToSelector:@selector(collectionNode:shouldDeselectItemAtIndexPath:)];
    _asyncDelegateFlags.collectionNodeDidDeselectItem =
        [_asyncDelegate respondsToSelector:@selector(collectionNode:didDeselectItemAtIndexPath:)];
    ...
    _asyncDelegateFlags.collectionNodeDidHighlightItem =
        [_asyncDelegate respondsToSelector:@selector(collectionNode:didHighlightItemAtIndexPath:)];
    _asyncDelegateFlags.collectionNodeDidUnhighlightItem =
        [_asyncDelegate respondsToSelector:@selector(collectionNode:didUnhighlightItemAtIndexPath:)];
    _asyncDelegateFlags.collectionNodeShouldShowMenuForItem =
        [_asyncDelegate respondsToSelector:@selector(collectionNode:shouldShowMenuForItemAtIndexPath:)];
    _asyncDelegateFlags.collectionNodeCanPerformActionForItem =
        [_asyncDelegate respondsToSelector:@selector(collectionNode:canPerformAction:forItemAtIndexPath:sender:)];
    ...
    _asyncDelegateFlags.collectionNodeWillDisplaySupplementaryElement =
        [_asyncDelegate respondsToSelector:@selector(collectionNode:willDisplaySupplementaryElementWithNode:)];
    _asyncDelegateFlags.collectionNodeDidEndDisplayingSupplementaryElement =
        [_asyncDelegate respondsToSelector:@selector(collectionNode:didEndDisplayingSupplementaryElementWithNode:)];
    ...
    if (_asyncDelegateFlags.interop) {
      ...
      _asyncDelegateFlags.interopWillDisplayCell =
          [interopDelegate respondsToSelector:@selector(collectionView:willDisplayCell:forItemAtIndexPath:)];
      _asyncDelegateFlags.interopDidEndDisplayingCell =
          [interopDelegate respondsToSelector:@selector(collectionView:didEndDisplayingCell:forItemAtIndexPath:)];
      _asyncDelegateFlags.interopWillDisplaySupplementaryView = [interopDelegate
          respondsToSelector:@selector(collectionView:willDisplaySupplementaryView:forElementKind:atIndexPath:)];
      _asyncDelegateFlags.interopdidEndDisplayingSupplementaryView = [interopDelegate
...
- (id<ASCollectionViewLayoutInspecting>)layoutInspector {
  if (_layoutInspector == nil) {
    ...
    ASDisplayNodeAssertNotNil(_defaultLayoutInspector,
                              @"You must not return nil from -asdk_layoutInspector. Return [super "
                              @"asdk_layoutInspector] if you have to! Layout: %@",
                              layout);
...
- (void)setLayoutInspector:(id<ASCollectionViewLayoutInspecting>)layoutInspector {
  ...
  _layoutInspectorFlags.constrainedSizeForSupplementaryNodeOfKindAtIndexPath = [_layoutInspector
      respondsToSelector:@selector(collectionView:constrainedSizeForSupplementaryNodeOfKind:atIndexPath:)];
  _layoutInspectorFlags.supplementaryNodesOfKindInSection =
      [_layoutInspector respondsToSelector:@selector(collectionView:supplementaryNodesOfKind:inSection:)];
  _layoutInspectorFlags.didChangeCollectionViewDataSource =
      [_layoutInspector respondsToSelector:@selector(didChangeCollectionViewDataSource:)];
  _layoutInspectorFlags.didChangeCollectionViewDelegate =
      [_layoutInspector respondsToSelector:@selector(didChangeCollectionViewDelegate:)];
...
- (CGSize)sizeForElement:(ASCollectionElement *)element {
  ...
  if (useUIKitCell) {
    ...
    if (wrapperNode.sizeForItemBlock) {
      ...
      // and call out to the delegate directly. If it has been deleted from the model, the size returned will be the
      // layout's default.
...
- (NSIndexPath *)convertIndexPathFromCollectionNode:(NSIndexPath *)indexPath waitingIfNeeded:(BOOL)wait {
  ...
  NSIndexPath *viewIndexPath = [_dataController.visibleMap convertIndexPath:indexPath
                                                                    fromMap:_dataController.pendingMap];
...
- (nullable NSIndexPath *)validateIndexPath:(nullable NSIndexPath *)indexPath {
  ...
  if (section >= self.numberOfSections) {
    ASDisplayNodeFailAssert(@"Collection view index path has invalid section %lu, section count = %lu",
                            (unsigned long)section, (unsigned long)self.numberOfSections);
    return nil;
  }
  ...
  if (item != NSNotFound && item >= [self numberOfItemsInSection:section]) {
    ASDisplayNodeFailAssert(@"Collection view index path has invalid item %lu in section %lu, item count = %lu",
                            (unsigned long)indexPath.item, (unsigned long)section,
                            (unsigned long)[self numberOfItemsInSection:section]);
    return nil;
  }
...
- (NSArray<NSIndexPath *> *)convertIndexPathsToCollectionNode:(NSArray<NSIndexPath *> *)indexPaths {
  return ASArrayByFlatMapping(indexPaths, NSIndexPath * viewIndexPath,
                              [self convertIndexPathToCollectionNode:viewIndexPath]);
}
...
- (CGSize)_sizeForUIKitCellWithKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath {
  ...
  if (kind == nil) {
    ASDisplayNodeAssert(_asyncDataSourceFlags.interop,
                        @"This code should not be called except for UIKit passthrough compatibility");
    ...
    ASDisplayNodeAssert(_asyncDataSourceFlags.interopViewForSupplementaryElement,
                        @"This code should not be called except for UIKit passthrough compatibility");
...
- (void)beginUpdates {
  ...
  if (_batchUpdateCount == 0) {
    ...
    _changeSet.rootActivity =
        as_activity_create("Perform async collection update", AS_ACTIVITY_CURRENT, OS_ACTIVITY_FLAG_DEFAULT);
    _changeSet.submitActivity =
        as_activity_create("Submit changes for collection update", _changeSet.rootActivity, OS_ACTIVITY_FLAG_DEFAULT);
  }
  _batchUpdateCount++;
}
...
- (void)endUpdatesAnimated:(BOOL)animated completion:(nullable void (^)(BOOL))completion {
  ...
  NSAssert(_batchUpdateCount >= 0,
           @"endUpdatesAnimated:completion: called without having a balanced beginUpdates call");
  ...
  if (_batchUpdateCount == 0) {
    ...
    // Nil out _changeSet before forwarding to _dataController to allow the change set to cause subsequent batch updates
...
- (void)registerSupplementaryNodeOfKind:(NSString *)elementKind {
  ...
  [self registerClass:[_ASCollectionReusableView class]
      forSupplementaryViewOfKind:elementKind
             withReuseIdentifier:kReuseIdentifier];
}
...
- (void)moveItemAtIndexPath:(NSIndexPath *)indexPath toIndexPath:(NSIndexPath *)newIndexPath {
  ...
  if (!_reordering) {
    [self
        performBatchUpdates:^{
          [_changeSet moveItemAtIndexPath:indexPath
                              toIndexPath:newIndexPath
                         animationOptions:kASCollectionViewAnimationNone];
        }
                 completion:nil];
  } else {
...
- (CGFloat)collectionView:(UICollectionView *)cv
  ...
  section = [self delegateIndexForSection:section withSelector:_cmd];
  if (section != NSNotFound) {
    return [(id)_asyncDelegate collectionView:cv layout:l minimumInteritemSpacingForSectionAtIndex:section];
  }
...
- (CGFloat)collectionView:(UICollectionView *)cv
  ...
  section = [self delegateIndexForSection:section withSelector:_cmd];
  if (section != NSNotFound) {
    return [(id)_asyncDelegate collectionView:cv layout:l minimumLineSpacingForSectionAtIndex:section];
  }
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView
  ...
  BOOL shouldDequeueExternally = _asyncDataSourceFlags.interopAlwaysDequeue ||
                                 (_asyncDataSourceFlags.interopViewForSupplementaryElement && wrapperNode);
  ...
  if (wrapperNode.viewForSupplementaryBlock) {
    ...
    view = [(id<ASCollectionDataSourceInterop>)_asyncDataSource collectionView:collectionView
                                             viewForSupplementaryElementOfKind:kind
                                                                   atIndexPath:indexPath];
  } else {
    ASDisplayNodeAssert(node != nil,
                        @"Supplementary node should exist.  Kind = %@, indexPath = %@, collectionDataSource = %@", kind,
                        indexPath, self);
    view = [self dequeueReusableSupplementaryViewOfKind:kind
                                    withReuseIdentifier:kReuseIdentifier
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView
  ...
  ASCellNode *node = element.node;
  ASWrapperCellNode *wrapperNode = (node.shouldUseUIKitCell ? (ASWrapperCellNode *)node : nil);
  BOOL shouldDequeueExternally =
      _asyncDataSourceFlags.interopAlwaysDequeue || (_asyncDataSourceFlags.interop && wrapperNode);

  if (wrapperNode.cellForItemBlock) {
    cell = wrapperNode.cellForItemBlock(wrapperNode);
  } else if (shouldDequeueExternally) {
    cell = [(id<ASCollectionDataSourceInterop>)_asyncDataSource collectionView:collectionView
                                                        cellForItemAtIndexPath:indexPath];
  } else {
    cell = [self dequeueReusableCellWithReuseIdentifier:kReuseIdentifier forIndexPath:indexPath];
  }

  ASDisplayNodeAssert(element != nil, @"Element should exist. indexPath = %@, collectionDataSource = %@", indexPath,
                      self);
  ASDisplayNodeAssert(cell != nil, @"UICollectionViewCell must not be nil. indexPath = %@, collectionDataSource = %@",
                      indexPath, self);
...
- (void)collectionView:(UICollectionView *)collectionView
       willDisplayCell:(UICollectionViewCell *)rawCell
    forItemAtIndexPath:(NSIndexPath *)indexPath {
  if (_asyncDelegateFlags.interopWillDisplayCell) {
    ASCellNode *node = [self nodeForItemAtIndexPath:indexPath];
    if (node.shouldUseUIKitCell) {
      [(id<ASCollectionDelegateInterop>)_asyncDelegate collectionView:collectionView
                                                      willDisplayCell:rawCell
  ...
  ASDisplayNodeAssertNotNil(
      cellNode, @"Expected node associated with cell that will be displayed not to be nil. indexPath: %@", indexPath);
...
- (void)collectionView:(UICollectionView *)collectionView
  ...
  // Retrieve the element from cell instead of visible map because at this point visible map could have been updated and
  // no longer holds the element.
  ASCollectionElement *element = cell.element;
  if (element) {
...
- (void)collectionView:(UICollectionView *)collectionView
    willDisplaySupplementaryView:(UICollectionReusableView *)rawView
  ...
  if (_asyncDelegateFlags.interopWillDisplaySupplementaryView) {
    ...
    if (node.shouldUseUIKitCell) {
      [(id<ASCollectionDelegateInterop>)_asyncDelegate collectionView:collectionView
                                         willDisplaySupplementaryView:rawView
  ...
  if (element) {
    ASDisplayNodeAssertTrue([_dataController.visibleMap supplementaryElementOfKind:elementKind
                                                                       atIndexPath:indexPath] == view.element);
    [_visibleElements addObject:element];
  } else {
    ASDisplayNodeAssert(NO, @"Unexpected nil element for willDisplaySupplementaryView: %@, %@, %@", rawView, self,
                        indexPath);
    return;
  }
  ...
  if (view.layoutAttributes == nil) {
    view.layoutAttributes = [collectionView layoutAttributesForSupplementaryElementOfKind:elementKind
                                                                              atIndexPath:indexPath];
  }
  ...
  if (_asyncDelegateFlags.collectionNodeWillDisplaySupplementaryElement) {
    ...
    ASCellNode *node = element.node;
    ASDisplayNodeAssert([node.supplementaryElementKind isEqualToString:elementKind],
                        @"Expected node for supplementary element to have kind '%@', got '%@'.", elementKind,
                        node.supplementaryElementKind);
    [_asyncDelegate collectionNode:collectionNode willDisplaySupplementaryElementWithNode:node];
  }
}

- (void)collectionView:(UICollectionView *)collectionView
    didEndDisplayingSupplementaryView:(UICollectionReusableView *)rawView
  ...
  if (_asyncDelegateFlags.interopdidEndDisplayingSupplementaryView) {
    ...
    if (node.shouldUseUIKitCell) {
      [(id<ASCollectionDelegateInterop>)_asyncDelegate collectionView:collectionView
                                    didEndDisplayingSupplementaryView:rawView
  ...
  if (element) {
    ...
    ASDisplayNodeAssert(NO, @"Unexpected nil element for didEndDisplayingSupplementaryView: %@, %@, %@", rawView, self,
                        indexPath);
    return;
  }
...
- (BOOL)collectionView:(UICollectionView *)collectionView
    shouldShowMenuForItemAtIndexPath:(nonnull NSIndexPath *)indexPath {
...
- (BOOL)collectionView:(UICollectionView *)collectionView
      canPerformAction:(nonnull SEL)action
  ...
  if (_asyncDelegateFlags.collectionNodeCanPerformActionForItem) {
    ...
    indexPath = [self convertIndexPathToCollectionNode:indexPath];
    if (indexPath != nil) {
      return [_asyncDelegate collectionNode:collectionNode
...
- (void)collectionView:(UICollectionView *)collectionView
         performAction:(nonnull SEL)action
    forItemAtIndexPath:(nonnull NSIndexPath *)indexPath
            withSender:(nullable id)sender {
...
- (BOOL)collectionView:(UICollectionView *)collectionView canMoveItemAtIndexPath:(NSIndexPath *)indexPath {
  ...
  // to propagate the "presentation data" element map (containing the speculative in-progress moves) to the layout
  // delegate, and this can cause exceptions to be thrown from UICV. For example, if you drag an item out of a section,
  ...
  // would be to capture the invalidation contexts that are sent during interactive moves and make our own data source
  // translator.
  if ([self.collectionViewLayout isKindOfClass:[ASCollectionLayout class]]) {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
      as_log_debug(ASCollectionLog(),
                   "Collection node item interactive movement is not supported when using a layout delegate. This "
                   "message will only be logged once. Node: %@",
                   ASObjectDescriptionMakeTiny(self));
    });
    return NO;
  }
...
- (void)collectionView:(UICollectionView *)collectionView
  ...
  ASDisplayNodeAssert(_asyncDataSourceFlags.collectionNodeMoveItem,
                      @"Should not allow interactive collection item movement if data source does not support it.");
  ...
  if (ASCollectionNode *collectionNode = self.collectionNode) {
    [_asyncDataSource collectionNode:collectionNode
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView
  ...
  _deceleratingVelocity = CGPointMake(contentOffset.x - ((targetContentOffset != NULL) ? targetContentOffset->x : 0),
  ...
  if (_asyncDelegateFlags.scrollViewWillEndDragging) {
    [_asyncDelegate scrollViewWillEndDragging:scrollView
                                 withVelocity:velocity
                          targetContentOffset:(targetContentOffset ?: &contentOffset)];
  }
}
...
- (BOOL)canBatchFetch {
  ...
  BOOL canFetch =
      _asyncDelegateFlags.collectionNodeWillBeginBatchFetch || _asyncDelegateFlags.collectionViewWillBeginBatchFetch;
...
#pragma clang diagnostic pop
  ...
  if (block == nil) {
    if (cell == nil || ASDynamicCast(cell, ASCellNode) == nil) {
      ...
      if (_asyncDataSourceFlags.interop) {
        ...
        ASDisplayNodeFailAssert(@"ASCollection could not get a node block for item at index path %@: %@, %@. If you "
                                @"are trying to display a UICollectionViewCell, make sure your dataSource conforms to "
                                @"the <ASCollectionDataSourceInterop> protocol!",
                                indexPath, cell, block);
  ...
  return ^{
    ...
    ASDisplayNodeAssert([node isKindOfClass:[ASCellNode class]], @"ASCollectionNode provided a non-ASCellNode! %@, %@",
                        node, strongSelf);
...
- (BOOL)dataController:(ASDataController *)dataController
  ...
  if (element.supplementaryElementKind == nil) {
    ...
    attributes = [self layoutAttributesForSupplementaryElementOfKind:element.supplementaryElementKind
                                                         atIndexPath:indexPath];
  }
...
- (ASCellNodeBlock)dataController:(ASDataController *)dataController
  ...
  if (_asyncDataSourceFlags.collectionNodeNodeBlockForSupplementaryElement) {
    ...
    block = [_asyncDataSource collectionNode:collectionNode
        nodeBlockForSupplementaryElementOfKind:kind
                                   atIndexPath:indexPath];
  }
  if (!block && !cell && _asyncDataSourceFlags.collectionNodeNodeForSupplementaryElement) {
    ...
    cell = [_asyncDataSource collectionNode:collectionNode
          nodeForSupplementaryElementOfKind:kind
                                atIndexPath:indexPath];
  }
...
#pragma clang diagnostic pop
  ...
  if (block == nil) {
    if (cell == nil || ASDynamicCast(cell, ASCellNode) == nil) {
      ...
      // Otherwise the CGSizeZero default will cause UIKit to not show it (so this isn't an error like the cellForItem
      // case).
...
- (NSArray<NSString *> *)dataController:(ASDataController *)dataController
  ...
  if (_asyncDataSourceFlags.collectionNodeSupplementaryElementKindsInSection) {
    ...
    [sections enumerateIndexesUsingBlock:^(NSUInteger section, BOOL *_Nonnull stop) {
      NSArray<NSString *> *kindsForSection = [_asyncDataSource collectionNode:collectionNode
                                           supplementaryElementKindsInSection:section];
...
- (ASSizeRange)dataController:(ASDataController *)dataController
  ...
  if (_layoutInspectorFlags.constrainedSizeForSupplementaryNodeOfKindAtIndexPath) {
    return [self.layoutInspector collectionView:self
        constrainedSizeForSupplementaryNodeOfKind:kind
                                      atIndexPath:indexPath];
  }

  ASDisplayNodeAssert(NO, @"To support supplementary nodes in ASCollectionView, it must have a layoutInspector for "
                          @"layout inspection. (See ASCollectionViewFlowLayoutInspector for an example.)");
  ...
  ASPerformBlockWithoutAnimation(!changeSet.animated, ^{
    ...
    if (changeSet.includesReloadData) {
      ...
      const auto completion = ^(BOOL finished) {
        as_activity_scope(as_activity_create("Handle collection update completion", changeSet.rootActivity,
      ...
      if (shouldReloadData) {
        ...
        [self
            _superPerformBatchUpdates:^{
              ...
              for (_ASHierarchyItemChange *change in
                   [changeSet itemChangesOfType:_ASHierarchyChangeTypeOriginalDelete]) {
                [super deleteItemsAtIndexPaths:change.indexPaths];
                numberOfUpdates++;
              }

              for (_ASHierarchySectionChange *change in
                   [changeSet sectionChangesOfType:_ASHierarchyChangeTypeOriginalDelete]) {
                [super deleteSections:change.indexSet];
                numberOfUpdates++;
              }

              for (_ASHierarchySectionChange *change in
                   [changeSet sectionChangesOfType:_ASHierarchyChangeTypeOriginalInsert]) {
                [super insertSections:change.indexSet];
                numberOfUpdates++;
              }

              for (_ASHierarchyItemChange *change in
                   [changeSet itemChangesOfType:_ASHierarchyChangeTypeOriginalInsert]) {
