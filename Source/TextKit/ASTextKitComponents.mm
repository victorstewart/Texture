...
@interface ASTextKitComponentsTextView () {
  ...
  // https://github.com/TextureGroup/Texture/issues/860
  BOOL _deallocating;
}
...
+ (instancetype)componentsWithTextStorage:(NSTextStorage *)textStorage
  ...
  components.textContainer.lineFragmentPadding =
      0.0;  // We want the text laid out up to the very edges of the text-view.
...
- (CGSize)sizeForConstrainedWidth:(CGFloat)constrainedWidth {
  ...
  // calculation. Otherwise, we create a temporary stack to size for `constrainedWidth`.
  if (CGRectGetWidth(components.textView.threadSafeBounds) != constrainedWidth) {
    components = [ASTextKitComponents componentsWithAttributedSeedString:components.textStorage
                                                       textContainerSize:CGSizeMake(constrainedWidth, CGFLOAT_MAX)];
  }
...
- (CGSize)sizeForConstrainedWidth:(CGFloat)constrainedWidth forMaxNumberOfLines:(NSInteger)maxNumberOfLines {
  ...
  while (lineRange.location < NSMaxRange(glyphRange)) {
    rect = [components.layoutManager lineFragmentRectForGlyphAtIndex:lineRange.location effectiveRange:&lineRange];
