...
+ (void)adjustFontSizeForAttributeString:(NSMutableAttributedString *)attrString withScaleFactor:(CGFloat)scaleFactor
{
  ...
  [attrString
      enumerateAttributesInRange:NSMakeRange(0, attrString.length)
                         options:0
                      usingBlock:^(NSDictionary<NSString *, id> *_Nonnull attrs, NSRange range, BOOL *_Nonnull stop) {
                        ...
                        if (attrs[NSKernAttributeName] != nil) {
                          ...
                          [attrString addAttribute:NSKernAttributeName
                                             value:@([kerning floatValue] * scaleFactor)
                                             range:range];
                        }
...
- (NSUInteger)lineCountForString:(NSAttributedString *)attributedString
{
  ...
  for (NSRange lineRange = {0, 0};
       NSMaxRange(lineRange) < [sizingLayoutManager numberOfGlyphs] && lineCount <= _attributes.maximumNumberOfLines;
       lineCount++) {
...
- (CGFloat)scaleFactor
{
  ...
  // actually determining if we need to scale at all. If something doesn't fit, we will continue to iterate our scale
  // factors.
  ...
  [_context performBlockWithLockedTextKitComponents:^(
    ...
    if (longestWordFits == NO) {
      ...
      longestWordSize = [attrString boundingRectWithSize:CGSizeMake(CGFLOAT_MAX, CGFLOAT_MAX)
                                                 options:NSStringDrawingUsesLineFragmentOrigin
                                                 context:nil]
                            .size;
    }

    // we may need to shrink for some reason, so let's iterate through our scale factors to see if we actually need to
    // shrink Note: the first scale factor in the array is 1.0 so will make sure that things don't fit without shrinking
    for (NSNumber *adjustedScaleObj in scaleFactors) {
      ...
      // if the longest word fits, go ahead and check max line and height. If it didn't fit continue to the next scale
      // factor
      if (longestWordFits == YES) {
        // scale our string by the current scale factor
        NSMutableAttributedString *scaledString =
            [[NSMutableAttributedString alloc] initWithAttributedString:textStorage];
