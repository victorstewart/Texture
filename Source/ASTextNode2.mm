 ...
 */
static NS_RETURNS_RETAINED ASTextLayout *ASTextNodeCompatibleLayoutWithContainerAndText(ASTextContainer *container,
  ...
  CGRect containerBounds = (CGRect){.size = container.size};
  {
    for (const auto &t : cacheValue->_layouts) {
      ...
      CGRect minRect = CGRectMake(0, 0, MIN(layoutSize.width, constrainedSize.width),
                                  MIN(layoutSize.height, constrainedSize.height));
      if (!CGRectContainsRect(containerBounds, minRect)) {
        continue;
      }
      CGRect maxRect = CGRectMake(0, 0, MAX(layoutSize.width, constrainedSize.width),
                                  MAX(layoutSize.height, constrainedSize.height));
...
- (NSString *)_plainStringForDescription {
  NSString *plainString =
      [[self.attributedText string] stringByTrimmingCharactersInSet:[NSCharacterSet newlineCharacterSet]];
...
- (BOOL)supportsLayerBacking {
  ...
  for (NSString *linkAttributeName in _linkAttributeNames) {
    ...
    [attributedText enumerateAttribute:linkAttributeName
                               inRange:range
...
- (CGSize)calculateSizeThatFits:(CGSize)constrainedSize {
  ASDisplayNodeAssert(constrainedSize.width >= 0, @"Constrained width for text (%f) is too  narrow",
                      constrainedSize.width);
  ASDisplayNodeAssert(constrainedSize.height >= 0, @"Constrained height for text (%f) is too short",
  ...
  // If the constrained size has a max/inf value on the text's forward direction, the text node is calculating its
  // intrinsic size. Need to consider both width and height when determining if it is calculating instrinsic size. Even
  // the constrained width is provided, the height can be inf it may provide a text that is longer than the width and
  // require a wordWrapping line break mode and looking for the height to be calculated.
  BOOL isCalculatingIntrinsicSize = (_textContainer.size.width >= ASTextContainerMaxSize.width) ||
                                    (_textContainer.size.height >= ASTextContainerMaxSize.height);
...
// Returns the ascender of the first character in attributedString by also including the line height if specified in
// paragraph style.
...
- (void)setAttributedText:(NSAttributedString *)attributedText {
  ...
  if (length > 0) {
    ...
    style.descender = [[attributedText attribute:NSFontAttributeName
                                         atIndex:attributedText.length - 1
                                  effectiveRange:NULL] descender];
  }
...
- (void)prepareAttributedString:(NSMutableAttributedString *)attributedString
             isForIntrinsicSize:(BOOL)isForIntrinsicSize {
  ASLockScopeSelf();
  ...
  [attributedString enumerateAttribute:NSParagraphStyleAttributeName
                            ...
                            usingBlock:^(NSParagraphStyle *style, NSRange range, BOOL *_Nonnull stop) {
                              ...
                              // Other alignments like "right", "center" and "natural" cause the size to be bigger than
                              // needed and thus should be ignored/overridden.
                              const BOOL forceLeftAlignment =
                                  (style != nil && isForIntrinsicSize && style.alignment != NSTextAlignmentLeft &&
  ...
  if (_shadowOpacity > 0 && (_shadowRadius != 0 || !CGSizeEqualToSize(_shadowOffset, CGSizeZero)) &&
      CGColorGetAlpha(_shadowColor) > 0) {
    NSShadow *shadow = [[NSShadow alloc] init];
    if (_shadowOpacity != 1) {
      CGColorRef shadowColorRef =
          CGColorCreateCopyWithAlpha(_shadowColor, _shadowOpacity * CGColorGetAlpha(_shadowColor));
...
- (NSObject *)drawParametersForAsyncLayer:(_ASDisplayLayer *)layer {
  ...
  return @{@"container" : copiedContainer, @"text" : mutableText, @"bgColor" : self.backgroundColor ?: [NSNull null]};
}

+ (void)drawRect:(CGRect)bounds
    withParameters:(NSDictionary *)layoutDict
  ...
  [layout drawInContext:context
                  ...
                  point:bounds.origin
                   view:nil
                  layer:nil
                  debug:[ASTextDebugOption sharedDebugOption]
                 cancel:isCancelledBlock];
}
...
- (id)_linkAttributeValueAtPoint:(CGPoint)point
  ...
  static constexpr CGSize kRectOffsets[9] = {{0, 0},     {-22, 0},  {22, 0},   {0, -22}, {0, 22},
  ...
  for (const CGSize &offset : kRectOffsets) {
    ...
    for (NSString *attributeName in _linkAttributeNames) {
      ...
      if (highlighting &&
          ...
          ![_delegate textNode:(ASTextNode *)self
              shouldHighlightLinkAttribute:attributeName
                                     value:value
                                   atPoint:point]) {
        continue;
      }
    ...
    if (stringIndexForPosition != kCFNotFound) {
      ...
      switch (_textContainer.truncationType) {
        case ASTextTruncationTypeStart: {
          CFIndex composedTruncationTextLineGlyphCount =
              truncationTokenLineGlyphCount + additionalTruncationTokenLineGlyphCount;
...
- (void)_setHighlightRange:(NSRange)highlightRange
          forAttributeName:(NSString *)highlightedAttributeName
                     value:(id)highlightedAttributeValue
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
  ...
  if (inAdditionalTruncationMessage) {
    ...
    [self _setHighlightRange:truncationMessageRange
            forAttributeName:ASTextNodeTruncationTokenAttributeName
                       value:nil
                    animated:YES];
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
  ...
  ASLockScopeSelf();  // Protect usage of _highlight* ivars.
  id<ASTextNodeDelegate> delegate = self.delegate;
  if ([self _pendingLinkTap] && [delegate respondsToSelector:@selector(textNode:
                                                                 tappedLinkAttribute:value:atPoint:textRange:)]) {
    CGPoint point = [[touches anyObject] locationInView:self.view];
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
  ...
  // on 3D Touch enabled phones, this gets fired with changes in force, and usually will get fired immediately after
  // touchesBegan:withEvent:
  if (CGPointEqualToPoint([touch previousLocationInView:self.view], locationInView)) return;
...
- (void)_handleLongPress:(UILongPressGestureRecognizer *)longPressRecognizer {
  ...
  if (longPressRecognizer.state == UIGestureRecognizerStateBegan) {
    ...
    if ([delegate respondsToSelector:@selector(textNode:longPressedLinkAttribute:value:atPoint:textRange:)]) {
      ...
      [delegate textNode:(ASTextNode *)self
          longPressedLinkAttribute:_highlightedLinkAttributeName
                             value:_highlightedLinkAttributeValue
                           atPoint:touchPoint
                         textRange:_highlightRange];
    }
...
static NSAttributedString *DefaultTruncationAttributedString() {
  ...
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    defaultTruncationAttributedString =
        [[NSAttributedString alloc] initWithString:NSLocalizedString(@"\u2026", @"Default truncation string")];
  });
...
- (NSAttributedString *)_locked_composedTruncationText {
  ...
  if (_composedTruncationText == nil) {
    if (_truncationAttributedText != nil && _additionalTruncationMessage != nil) {
      NSMutableAttributedString *newComposedTruncationString =
          [[NSMutableAttributedString alloc] initWithAttributedString:_truncationAttributedText];
...
- (NSAttributedString *)_locked_prepareTruncationStringForDrawing:(NSAttributedString *)truncationString {
  ...
  if (_attributedText.length > 0) {
    ...
    NSDictionary *originalStringAttributes = [originalString attributesAtIndex:originalStringLength - 1
                                                                effectiveRange:NULL];
    [truncationString enumerateAttributesInRange:NSMakeRange(0, truncationString.length)
                                         options:0
                                      usingBlock:^(NSDictionary *attributes, NSRange range, BOOL *stop) {
                                        NSMutableDictionary *futureTruncationAttributes =
                                            [originalStringAttributes mutableCopy];
